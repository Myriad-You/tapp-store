...
包内保留上海线路 JSON、网页版高清 PNG 和参考 APK 中的 PDF，断网仍可查询；北京另有精简基线。选择城市时，应用从稳定的 MetroMan 官方换乘页下载有序线路和站点数据；仅对 MetroMan 实际支持新版的四个城市额外请求新版换乘页，因此其他城市不会先触发新版页面的 500 错误。应用同时检查对应城市高清线路图。所有在线线路图 URL 都经过 HTTPS 与官方主机白名单校验，城市 ID 和站点 slug 也会在代入请求路径前校验。顶栏会持续显示 MetroMan 的连接、权限或 API 注册状态，不再静默吞掉网络错误。在线数据必须通过完整结构校验才会写入私有缓存；任何失败都不会覆盖或删除离线数据，也不会上传用户输入的站点。
...
应用代码与自有界面代码按仓库声明使用 MIT；MetroMan 网站返回的线路图、线路数据、票价与时刻信息，以及参考 APK 中保留的上海 PDF，均属于 MetroMan 或其相关权利方的外部资源，不由本项目授予 MIT 许可。应用仅通过 MetroMan 官方页面和声明式 API 读取在线资源，并在界面和本说明中保留来源提示。

## 在己方服务测试

1. 打开 https://myriad.360nenz.top/，登录后进入 Tapp 管理页。
2. 选择上传本地打包得到的 `1.3.16` `.tapp` 文件，确认私有存储、通知、打开声明链接和 `network:fetch` 权限均已授予。当前包使用 Myriad 新版 `core.entry` 与 `page.template/styles` 声明层。联网检查直接调用已声明的 MetroMan API，Bridge 请求串行化并由超时保护，启动不会因缓存读取耗尽宿主并发额度。
...
9. 切换到“线路图”，确认该页城市选择器与路线规划同步；测试滚轮指针中心缩放至 500%、按住拖动、双击复位、缩放按钮，以及上海专属“打开参考 PDF”。
...
本分支从提交 d133f6fd59c2e46c9c3cc08725d6dc7933578b4c 创建；当前包版本为 1.3.16。建议使用中文 Conventional Commit，并启用 GPG 签名：

    git add apps/top.360nenz.metro-guide
