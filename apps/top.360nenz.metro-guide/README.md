...
2. 选择上传本地打包得到的 1.3.1 `.tapp` 文件，确认私有存储、通知、打开声明链接和 `network:fetch` 权限均已授予。当前包继续使用 Myriad 0.3.x 的 `main/hasPage/pageTemplate/pageStyles` Manifest 字段；在 Myriad 0.4.0 正式发布前，不使用尚不兼容的 `core/page` 声明层。
...
10. 点击“网页高清 PNG”确认宿主打开当前城市的 MetroMan 线路图页面；点击“更新数据”，顶栏应依次显示连接中和已连接。若显示权限未授予，请到 Tapp 权限页重新批准 `network:fetch`；若显示 API 未注册，请确认已更新到 1.3.1。
...
本分支从提交 d133f6fd59c2e46c9c3cc08725d6dc7933578b4c 创建；当前包版本为 1.3.1。建议使用中文 Conventional Commit，并启用 GPG 签名：
