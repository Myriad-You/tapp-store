...
2. 选择上传本地打包得到的 1.3.4 `.tapp` 文件，确认私有存储、通知、打开声明链接和 `network:fetch` 权限均已授予。当前包继续使用 Myriad 0.3.x 的 `main/hasPage/pageTemplate/pageStyles` Manifest 字段；在 Myriad 0.4.0 正式发布前，不使用尚不兼容的 `core/page` 声明层。联网检查直接调用已声明的 MetroMan API，单次请求最多等待 12 秒；启动先加载包内离线资源，城市缓存随后后台读取，不会阻塞网络请求。
...
本分支从提交 d133f6fd59c2e46c9c3cc08725d6dc7933578b4c 创建；当前包版本为 1.3.4。建议使用中文 Conventional Commit，并启用 GPG 签名：
