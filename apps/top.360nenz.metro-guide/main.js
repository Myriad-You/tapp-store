...
function legacyClearRouteMeta() { ['[data-duration]','[data-route] [data-fare]','[data-distance]','[data-result-context]'].forEach(function(selector) { document.querySelector(selector).textContent = ''; }); }
...
async function loadCachedCities(cityId) {
  ...
  if (!metroMapCacheLoaded) {
    ...
    if (mapCache && mapCache.cities) Object.keys(mapCache.cities).forEach(function(mapCityId) { if (mapCache.cities[mapCityId] && mapCache.cities[mapCityId].url) metroState.mapUrls[mapCityId] = mapCache.cities[mapCityId].url; });
