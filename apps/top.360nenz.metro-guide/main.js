...
var metroState = { city:'beijing', favorites:[], zoom:100, panX:0, panY:0, dragging:false, dragStart:null, assetUrls:{}, mapUrls:{}, versions:{}, cityCache:{}, loadingCity:'', mapVersion:'', cityRequest:0, routeRequest:0, stationTarget:'from', stationLineFilter:'', mode:'depart', fare:'ic', preference:'time' };
...
var METROMAN_HOSTS = ['www.metroman.cn', 'metroman.cn', 'metroman.oss-cn-hangzhou.aliyuncs.com'];
...
function isKnownCityId(id) { return typeof id === 'string' && METRO_CITIES.some(function(city) { return city.id === id; }); }
function safeCityId(id) { if (!isKnownCityId(id)) throw new Error('无效 MetroMan 城市'); return id; }
function safeStationSlug(slug) { return typeof slug === 'string' && /^[a-z0-9]+(?:-[a-z0-9]+)*$/i.test(slug) ? slug : ''; }
function safePlannerRoute(route) {
  if (typeof route !== 'string') return '';
  var parts = route.split('-to-');
  return parts.length === 2 && safeStationSlug(parts[0]) && safeStationSlug(parts[1]) ? route : '';
}
function safeMetroMapUrl(value, allowLocal) {
  if (typeof value !== 'string' || !value.trim()) return '';
  if (allowLocal && /^(blob:|tapp:|data:)/i.test(value)) return value;
  try {
    var url = new URL(value, METRO_MAP_PAGE);
    if (url.protocol !== 'https:' || METROMAN_HOSTS.indexOf(url.hostname.toLowerCase()) < 0) return '';
    if (!/routemap_[a-z0-9_]+\.png$/i.test(url.pathname)) return '';
    return url.href;
  } catch (_) { return ''; }
}
...
var metroPlanCache = Object.create(null);
var metroPlanInflight = Object.create(null);
var metroCityCacheInflight = Object.create(null);
var metroMapCacheLoaded = false;
var metroLegacyCacheLoaded = false;
var metroRecentSelection = null;
var metroRecentSelectionInflight = null;
var METRO_PLAN_CACHE_TTL = 60000;
...
function metroApi(name, params) {
  return queuedBridge(function() {
    ...
    var value = Object.assign({}, params || {});
    if (Object.prototype.hasOwnProperty.call(value, 'city')) value.city = safeCityId(value.city);
    if (Object.prototype.hasOwnProperty.call(value, 'route')) {
      value.route = safePlannerRoute(value.route);
      if (!value.route) throw new Error('无效 MetroMan 站点路径');
    }
    switch (name) {
...
function metroPlanApi(name, params) {
  var key = name + '|' + JSON.stringify(params || {});
  var cached = metroPlanCache[key];
  if (cached && Date.now() - cached.savedAt < METRO_PLAN_CACHE_TTL) return Promise.resolve(cached.value);
  if (metroPlanInflight[key]) return metroPlanInflight[key];
  metroPlanInflight[key] = metroApi(name, params).then(function(value) {
    delete metroPlanInflight[key];
    metroPlanCache[key] = { savedAt:Date.now(), value:value };
    return value;
  }, function(error) {
    delete metroPlanInflight[key];
    throw error;
  });
  return metroPlanInflight[key];
}
...
async function loadCachedCities(cityId) {
  if (cityId === 'shanghai' && !metroLegacyCacheLoaded) {
    metroLegacyCacheLoaded = true;
  ...
  if (cityId && cityId !== 'shanghai') {
    var cached = await storageGet('metro-guide.city.' + cityId + '.v1', null);
    if (validCity(cached)) METRO_DATA[cityId] = cached;
  }
  if (!metroMapCacheLoaded) {
    metroMapCacheLoaded = true;
    var mapCache = await storageGet(METRO_MAP_CACHE_KEY, null);
    if (mapCache && mapCache.cities) Object.keys(mapCache.cities).forEach(function(mapCityId) { var safeUrl = safeMetroMapUrl(mapCache.cities[mapCityId] && mapCache.cities[mapCityId].url, false); if (isKnownCityId(mapCityId) && safeUrl) metroState.mapUrls[mapCityId] = safeUrl; });
    else if (mapCache && mapCache.url) { var legacyUrl = safeMetroMapUrl(mapCache.url, false); var legacyCityId = mapCache.cityId; if (legacyUrl && isKnownCityId(legacyCityId)) metroState.mapUrls[legacyCityId] = legacyUrl; }
  }
  if (document.querySelector('[data-app]')) { renderCityOptions(); if (metroState.city) { renderStations(); renderLines(); } }
}
function loadCityCache(cityId) {
  if (!cityId || metroCityCacheInflight[cityId]) return metroCityCacheInflight[cityId] || Promise.resolve();
...
function extractOnlineMap(html, cityId) { var safeId = safeCityId(cityId || metroState.city); var match = String(html || '').match(/(?:url:\s*['"]|src=['"])([^'"]*routemap_[a-z0-9_]+\.png[^'"]*)['"]/i); if (!match) match = String(html || '').match(/([^'"\s]*routemap_[a-z0-9_]+\.png)/i); if (!match) return null; return safeMetroMapUrl(match[1], false) || safeMetroMapUrl(new URL(match[1], METRO_MAP_PAGE + safeId).href, false) || null; }
...
async function updateOnline(silent, requestedCity, requestToken, backgroundRun) {
  ...
  try {
    ...
    if (pageImage) {
      var safePageImage = safeMetroMapUrl(pageImage, false);
      if (!safePageImage) throw new Error('MetroMan 线路图地址不受信任');
      metroState.mapUrls[cityId] = safePageImage; metroState.mapVersion = new Date().toISOString(); mapChecked = true;
      ...
      mapMeta.cities = mapMeta.cities || {}; mapMeta.cities[cityId] = { url:safePageImage, checkedAt:metroState.mapVersion, source:'MetroMan' };
      await storageSet(METRO_MAP_CACHE_KEY, mapMeta);
      if (metroState.city === cityId && (!requestToken || requestToken === metroState.cityRequest)) document.querySelector('[data-map-image]').src = safePageImage;
    }
...
function loadRecentSelection() {
  if (metroRecentSelection) return Promise.resolve(metroRecentSelection);
  if (metroRecentSelectionInflight) return metroRecentSelectionInflight;
  metroRecentSelectionInflight = storageGet(METRO_RECENT_KEY, null).then(function(value) {
    metroRecentSelection = value && typeof value === 'object' ? value : recentSelection();
    metroRecentSelectionInflight = null;
    return metroRecentSelection;
  });
  return metroRecentSelectionInflight;
}
function saveRecentSelection(cityId) { var root = document.querySelector('[data-app]'); var value = { city:cityId || metroState.city, from:root.querySelector('[data-from]').value.trim(), to:root.querySelector('[data-to]').value.trim() }; metroRecentSelection = value; setTimeout(function() { storageSet(METRO_RECENT_KEY, value); }, 0); return Promise.resolve(); }
...
function lineIndexName(line) { return String(line.id || '').trim(); }
...
function plannerStationSlug(name) { var slug = (data().stationSlugs && data().stationSlugs[name]) || COMMON_STATION_SLUGS[name] || ''; return safeStationSlug(slug); }

async function search() {
  var from = document.querySelector('[data-from]').value.trim();
  var to = document.querySelector('[data-to]').value.trim();
  var requestId = ++metroState.routeRequest;
  ...
  if (!fromSlug || !toSlug || !isKnownCityId(metroState.city)) return;
  ...
  try {
    var result = isV2 ? await metroPlanApi('metroPlannerV2Result', params) : await metroPlanApi('metroPlannerResult', params);
    if (requestId !== metroState.routeRequest) return;
    var html = responseHtml(result);
    var plan = isV2 ? parseOfficialPlan(html) : parseLegacyPlan(html);
    if (plan) {
      plan.isV2 = isV2;
      renderOfficialPlan(plan, from, to);
    } else {
      var officialFare = extractOfficialFare(html);
      if (officialFare) document.querySelector('[data-result-fare]').textContent = officialFare;
    }
  } catch (_) {
    if (requestId === metroState.routeRequest) notify('MetroMan 官方详情暂不可用，已保留本地少换乘方案。', 'warning');
  }
}
